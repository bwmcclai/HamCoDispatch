/* ===================================
   HAMILTON COUNTY DISPATCH — Backend
   HamCoDispatch.com
   with 10-code translation & geocoding
   =================================== */
...
// ── Hamilton County-wide Geocoding ──
// City center fallbacks for agencies
const CITY_CENTERS = {
...
// Major streets across Hamilton County
const HAMCO_GEO = {
    // ── NOBLESVILLE ──
...
async function fetchIncidents() {
    try {
        ...
        console.log(`[${new Date().toLocaleTimeString()}] Fetched ${rawIncidents.length} incidents from Hamilton County`);
        ...
        const transformed = rawIncidents
            .map(raw => {
                ...
                return {
                    ...
                    isNoblesville: ['Noblesville Fire', 'Noblesville Police'].includes(raw.Agency),
                };
            })
            .filter(i => i !== null)
        ...
        const cities = {};
        transformed.forEach(i => { cities[i.city] = (cities[i.city] || 0) + 1; });
        console.log(`[${new Date().toLocaleTimeString()}] Breakdown:`, JSON.stringify(cities));
...
app.get('/api/incidents', async (req, res) => {
    ...
    let result = [...cachedIncidents];

    // Filter by agency scope
    const agency = req.query.agency;
    if (agency === 'noblesville') {
        result = result.filter(i => i.isNoblesville);
    }
    ...
    if (category && ['fire', 'police', 'ems'].includes(category)) {
...
app.get('/api/stations', (req, res) => {
    res.json({ success: true, stations: FIRE_STATIONS });
});

// 10-code lookup
app.get('/api/codes', (req, res) => {
    res.json({ success: true, codes: TEN_CODES });
});
...
app.get('/api/health', (req, res) => {
    res.json({
        status: 'ok',
        cachedIncidents: cachedIncidents.length,
